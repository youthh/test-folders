...
зворотним відліком до побачення (`Countdown.tsx`), кнопкою «надіслати»
(Web Share API, а якщо його немає — копіює текст у буфер обміну) і
кнопкою «Додати в календар» — формує `.ics`-файл на льоту без жодного
бекенду (`ics.ts`) і одразу пропонує зберегти. Якщо для обраних ідей
побачення в `config.placeIdeas` є місця — під підсумком з'являється блок
«куди підемо» з посиланнями на карту.
